...
if source.count(old_lookup) != 1:
    raise SystemExit('expected one duplicate price lookup block')
...
if source.count(old_button) != 1:
    raise SystemExit('expected one Creator paywall product button')
source = source.replace(old_button, new_button, 1)

if '?? product.price' in source:
    raise SystemExit('ProductDetails.price fallback remains')
