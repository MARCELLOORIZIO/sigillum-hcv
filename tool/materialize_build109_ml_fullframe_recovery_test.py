...
test_path = Path('test/build109_ml_fullframe_recovery_test.dart')
if test_path.exists():
    raise SystemExit('BUILD109 ML recovery test already exists before materialization')
...
void main() {
  ...
  test('BUILD109 preserves the existing strict content-area 85 path', () {
    final result = _resolve(
      _ml(content: const <int>[90, 90, 59]),
    );
    expect(result.decision, 'STRONG_DISPLAY_RISK');
    expect(
      result.reasons,
  ...
  test('BUILD109 recovery requires two content-area samples at least 75', () {
    final result = _resolve(
      _ml(content: const <int>[75, 74, 59]),
    );
  ...
  test('BUILD109 recovery requires all three frame probabilities at least 0.90', () {
    final result = _resolve(
  ...
  test('BUILD109 recovery requires two full-frame scores at least 90', () {
    final result = _resolve(
      _ml(fullFrame: const <int>[96, 89, 89]),
    );
    expect(result.decision, 'NO_DISPLAY_EVIDENCE');
    expect(
      result.reasons,
  ...
  test('BUILD109 recovery requires all three aggregate strong screen frames', () {
...
''')
print('BUILD109 ML recovery test materialized')
