...
void main() {
  test(
    'periodic row phase sequence is detected as stable periodic structure',
    () {
      const bins = 96;
      const frames = 24;
      const spatialBin = 8;
      ...
      expect(
        (result['dominantRowFrequencyBin'] as int),
        inInclusiveRange(7, 9),
      );
      expect(
        (result['dominantFrequencyStability'] as double),
        greaterThan(0.80),
      );
      expect((result['phaseStepConsistency'] as double), greaterThan(0.80));
      expect(
        (result['medianTemporalDifferenceRms'] as double),
        greaterThan(0.005),
      );
    },
  );
  ...
  test(
    'scalar luminance modulation exposes temporal spectral concentration',
    () {
      ...
      expect(result['analysisStatus'], 'ANALYZED');
      expect(
        (result['robustFrameLumaModulationDepth'] as double),
        greaterThan(0.15),
      );
      expect(
        (result['temporalSpectralConcentration'] as double),
        greaterThan(0.90),
      );
    },
  );

  test('probe contract exposes the strict coherent HFR decision role', () {
