...
void main() {
  ...
  test('published social text uses visible HCV-ID and has a verification page', () {
    final source = File('lib/text_cert_page.dart').readAsStringSync();
    final verifier = File('lib/text_social_verify_page.dart').readAsStringSync();
  ...
  test('iOS text HCVPACK picker stays selectable and validates extension after selection', () {
    ...
    expect(verifier, contains("p.extension(packagePath).toLowerCase() != '.hcvpack'"));
  ...
  test('text workflow patch cannot alter camera or display classifiers', () {
    final patch = File('tool/apply_text_social_verification.py').readAsStringSync();
