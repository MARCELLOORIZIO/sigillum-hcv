...
void main() {
  test('text pages dismiss the iOS keyboard before actions and navigation', () {
    final script = File('tool/apply_text_keyboard_registry_photo_arm_fix.py')
        .readAsStringSync();
    expect(script, contains("SystemChannels.textInput.invokeMethod<void>('TextInput.hide')"));
  ...
  test('missing text certificate is recovered only from a valid signed local copy', () {
    final script = File('tool/apply_text_keyboard_registry_photo_arm_fix.py')
        .readAsStringSync();
  ...
  test('photo proceed only arms a manual shot while video flow is untouched', () {
    final script = File('tool/apply_text_keyboard_registry_photo_arm_fix.py')
        .readAsStringSync();
