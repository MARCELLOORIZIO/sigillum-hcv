# SIGILLUM Privacy Policy Draft

Draft for App Store publication. Before publishing, replace company details, public URLs, retention times and jurisdiction with final legal information.

## Italiano

SIGILLUM consente di creare e verificare contenuti associati a un HCV-ID. L'app tratta i contenuti creati o selezionati dall'utente, l'impronta crittografica del file, metadati tecnici necessari alla verifica, certificati HCV e dati di identita tecnica del creatore quando configurati.
...
SIGILLUM allows users to create and verify content associated with an HCV-ID. The app processes content created or selected by the user, the cryptographic file fingerprint, technical metadata required for verification, HCV certificates and technical creator identity data when configured.
...
SIGILLUM permite crear y verificar contenidos asociados a un HCV-ID. La app trata contenidos creados o seleccionados por el usuario, la huella criptografica del archivo, metadatos tecnicos necesarios para la verificacion, certificados HCV y datos de identidad tecnica del creador cuando estan configurados.
...
SIGILLUM позволяет создавать и проверять контент, связанный с HCV-ID. Приложение обрабатывает контент, созданный или выбранный пользователем, криптографический отпечаток файла, технические метаданные, необходимые для проверки, сертификаты HCV и данные технической идентичности автора, если они настроены.
